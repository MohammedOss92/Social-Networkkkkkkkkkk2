...
def profile(request, username):
    ...
    if request.user.is_authenticated:
        ...
        if request.user in Follower.objects.filter(user=user).values_list('followers', flat=True):
            follower = True
...
def following_lisat(request, username):
    user = get_object_or_404(User, username=username)
    
    # الحصول على الأشخاص الذين يتابعهم المستخدم
    following_users = user.user_following.all()
...
def follower_view(request):
    # استرجاع كائن Follower للمستخدم الحالي
    try:
        follower_object = Follower.objects.get(user=request.user)

        # الحصول على قائمة المستخدمين الذين يتابعهم المستخدم الحالي
        following_users = follower_object.followers.all()
